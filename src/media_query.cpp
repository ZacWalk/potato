...
std::shared_ptr<media_query> media_query::create_from_string(const std::wstring& str)
{
    auto query = std::make_shared<media_query>();
    auto tokens = split_string(str, _t(" \t\r\n"), _t("("));
    ...
    for (auto tok : tokens)
    {
        ...
        else if (tok.at(0) == _t('('))
        {
            ...
            auto expr_tokens = split_string(tok, L':');
            ...
            if (!expr_tokens.empty())
            {
                ...
                if (expr.feature != media_feature_none)
                {
                    ...
                    else
                    {
                        ...
                        {
                            auto slash_pos = expr_tokens[1].find_first_of(_t('/'));

                            if (slash_pos != std::wstring::npos)
                            {
                                std::wstring val1 = expr_tokens[1].substr(0, slash_pos);
                                std::wstring val2 = expr_tokens[1].substr(slash_pos + 1);
                                trim(val1);
                                trim(val2);
                                expr.val = std::stoi(val1);
                                expr.val2 = std::stoi(val2);
                            }
                            else
                            {
                                css_length length;
...
std::shared_ptr<media_query_list> media_query_list::create_from_string(const std::wstring& str)
{
    auto list = std::make_shared<media_query_list>();
    auto tokens = split_string(str, L',');
