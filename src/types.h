...
namespace Sizing
{
    // (Type)Safe array item counting - will fail to compile when used on a pointer or a heap object
    template < typename TDataType, size_t t_nElements >
    char(&lengthof_impl(TDataType const volatile (&)[t_nElements]))[t_nElements];

    template < typename TDataType, size_t t_nElements >
    char(&string_lengthof_impl(TDataType const volatile (&)[t_nElements]))[t_nElements - 1];
...
const wchar_t* const void_elements = _t("area;base;br;col;command;embed;hr;img;input;keygen;link;meta;param;source;track;wbr");
